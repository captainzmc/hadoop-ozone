...
public abstract class OMClientRequest implements RequestAuditor {
   ...
   * @param omResponse
   * @param ex         - IOException
   * @param unRenamedKeys    - Set<OmKeyInfo>
   * @return error response need to be returned to client - OMResponse.
   */
  protected OMResponse createRenameKeysErrorOMResponse(
      @Nonnull OMResponse.Builder omResponse,
      @Nonnull IOException ex, @Nonnull Set<OmKeyInfo> unRenamedKeys) {
    omResponse.setSuccess(false);
    StringBuffer errorMsg = new StringBuffer();
    RenameKeysResponse.Builder resp = RenameKeysResponse.newBuilder();
    for (OmKeyInfo key : unRenamedKeys) {
      if(key != null) {
    ...
    // TODO: Currently all rename operations in OzoneBucket.java are void. Here
    //  we put the List of unRenamedKeys into Response. These KeyInfo can be
    //  used to continue deletion if client support rename retry.
